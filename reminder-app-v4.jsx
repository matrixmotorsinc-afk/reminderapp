...
<title>My Health & Reminders</title>
...
<body style="margin:0;background:#0f0e17;">
<div id="root"></div>
...
const APPT_ICONS   = {Doctor:"👨‍⚕️",Dentist:"🦷","Eye Doctor":"👁️",Specialist:"🔬","Lab/Blood Work":"🩸","Imaging (X-Ray/MRI)":"🩻",Therapy:"🧠",Other:"🏥"};
...
function isOverdue(ds) {
  if (!ds) return false;
  const [y,m,d]=ds.split("-").map(Number);
  const t=new Date(); const ty=t.getFullYear(),tm=t.getMonth()+1,td=t.getDate();
...
function getAdvanceAlert(ds,advance) {
  ...
  const days=daysUntil(ds); if(days===null)return null;
  for(const a of advance){
    if(a==="1hour"&&days===0)  return "⏰ 1-hour reminder active";
    if(a==="1day" &&days===1)  return "⏰ Due tomorrow";
...
function App() {
  ...
  const [reminders,setReminders]= useState(()=>store.get("woa_reminders")||[]);
  const [bills,setBills]        = useState(()=>store.get("woa_bills")||[]);
  const [appts,setAppts]        = useState(()=>store.get("woa_appts")||[]);
  const [meds,setMeds]          = useState(()=>store.get("woa_meds")||[]);
  ...
  useEffect(()=>store.set("woa_reminders",reminders),[reminders]);
  useEffect(()=>store.set("woa_bills",bills),[bills]);
  useEffect(()=>store.set("woa_appts",appts),[appts]);
  useEffect(()=>store.set("woa_meds",meds),[meds]);
  ...
  }).sort((a,b)=>(!a.dueDate?1:!b.dueDate?-1:a.dueDate.localeCompare(b.dueDate)));
  ...
  const TABS=[["reminders","🔔","Reminders",rAlerts],["bills","💳","Bills",bAlerts],["appointments","🏥","Appts",aAlerts],["medications","💊","Meds",mAlerts]];

  return (
    <div style={{minHeight:"100vh",background:"#0f0e17",fontFamily:"'Lora','Georgia',serif",color:"#fffffe"}}>
      <style>{`
        *{box-sizing:border-box;}::placeholder{color:#4a4a6a;}
        ...
        .del{opacity:0;transition:opacity 0.15s;}.card:hover .del,.card:active .del{opacity:1;}
      ...
      {/* HEADER */}
      <div style={{borderBottom:"1px solid #1a1a2e",padding:"20px 20px 0",position:"sticky",top:0,background:"#0f0e17",zIndex:50}}>
        <div style={{maxWidth:680,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div>
              <div style={{fontSize:10,letterSpacing:4,color:"#ff8906",textTransform:"uppercase",marginBottom:2,fontFamily:"'DM Sans',sans-serif"}}>Your Space</div>
              <h1 style={{margin:0,fontSize:22,fontWeight:500,letterSpacing:-0.5}}>
                {tab==="reminders"?"Reminders":tab==="bills"?"Bills & Payments":tab==="appointments"?"Doctor & Lab":"Medications"}
              </h1>
            </div>
          ...
          <div style={{display:"flex"}}>
            {TABS.map(([t,icon,lbl,al])=>(
              <button key={t} onClick={()=>{setTab(t);setFilter("Active");setSearch("");setShowForm(false);setShowDetail(null);}}
                style={{flex:1,background:"transparent",border:"none",borderBottom:`2px solid ${tab===t?tabColor[t]:"transparent"}`,padding:"10px 0",color:tab===t?"#fffffe":"#5a5a7a",fontSize:11,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"all 0.2s",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
                <span style={{fontSize:16}}>{icon}</span>
                <span style={{fontSize:11}}>{lbl}</span>
                {al>0&&<span className="adot" style={{display:"inline-block",width:6,height:6,borderRadius:"50%",background:"#ef4444",flexShrink:0}}/>}
              </button>
            ))}
          </div>
        </div>
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"14px 20px 0"}}>
        <div style={{position:"relative",marginBottom:10}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search…" style={{...IS,paddingLeft:34,borderRadius:12,fontFamily:"'DM Sans',sans-serif"}}/>
        </div>
        {tab!=="appointments"&&(
          <div style={{display:"flex",gap:4,marginBottom:14}}>
            {["Active","All","Completed"].map(f=>(
              <button key={f} onClick={()=>setFilter(f)} className="pill"
                style={{background:filter===f?"#2a2940":"transparent",border:"none",borderRadius:8,padding:"5px 12px",color:filter===f?"#fffffe":"#5a5a7a",fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"0 20px 80px"}}>
        ...
        {/* ── REMINDERS ── */}
        {tab==="reminders"&&(filtR.length===0?(
          <div className="pulse" style={{textAlign:"center",padding:"60px 0",color:"#4a4a6a"}}>
            <div style={{fontSize:40,marginBottom:8}}>○</div>
            <div style={{fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>{search?"No matches":filter==="Completed"?"Nothing completed yet":"No reminders — tap + to add one"}</div>
          </div>
        ):(
              ...
              return(
                <div key={r.id} className="card fade" style={{background:"#13131f",border:`1px solid ${over?"#7f1d1d":r.completed?"#1a1a2e":"#2a2940"}`,borderLeft:`3px solid ${r.completed?"#2a2940":p.color}`,borderRadius:12,padding:"12px 14px",display:"flex",gap:10,alignItems:"flex-start",opacity:r.completed?0.5:1}}>
                  ...
                  <div style={{flex:1,minWidth:0,cursor:r.completed?"default":"pointer"}} onClick={()=>!r.completed&&openEditR(r)}>
                    ...
                    {r.note&&<div style={{fontSize:12,color:"#5a5a7a",marginBottom:4,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontFamily:"'DM Sans',sans-serif"}}>{r.note}</div>}
                    <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                      <span style={{fontSize:11,color:p.color,background:p.dim+"55",padding:"2px 8px",borderRadius:20,fontFamily:"'DM Sans',sans-serif"}}>{p.label}</span>
                      {r.dueDate&&<span style={{fontSize:11,color:over?"#ef4444":"#5a5a7a",fontFamily:"'DM Sans',sans-serif"}}>{over?"⚠ Overdue · ":""}{formatDate(r.dueDate)}</span>}
                      {adv&&<span style={{fontSize:11,color:"#f59e0b",background:"rgba(245,158,11,0.12)",padding:"2px 8px",borderRadius:20,fontFamily:"'DM Sans',sans-serif"}}>{adv}</span>}
                    </div>
                    {(r.advance||[]).length>0&&!r.completed&&r.dueDate&&<div style={{fontSize:10,color:"#3a3a5a",marginTop:3,fontFamily:"'DM Sans',sans-serif"}}>🔔 {r.advance.map(a=>ADVANCE_OPTIONS.find(x=>x.value===a)?.label).join(" · ")}</div>}
                  </div>
        ...
        {/* ── BILLS ── */}
        {tab==="bills"&&(filtB.length===0?(
          <div className="pulse" style={{textAlign:"center",padding:"60px 0",color:"#4a4a6a"}}>
            <div style={{fontSize:40,marginBottom:8}}>💳</div>
            <div style={{fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>{search?"No matches":filter==="Completed"?"No paid bills":"No bills — tap + to add one"}</div>
          </div>
        ):(
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {filtB.map(b=>{
                ...
                <div key={b.id} className="card fade" style={{background:"#13131f",border:`1px solid ${over?"#7f1d1d":urgent?"#78350f":b.paid?"#1a1a2e":"#2a2940"}`,borderLeft:`3px solid ${b.paid?"#2a2940":over?"#ef4444":urgent?"#f59e0b":"#10b981"}`,borderRadius:12,padding:"12px 14px",opacity:b.paid?0.5:1}}>
                  <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                    ...
                    <div style={{flex:1,minWidth:0,cursor:b.paid?"default":"pointer"}} onClick={()=>!b.paid&&openEditB(b)}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:2}}>
                        ...
                        {b.amount&&<div style={{fontSize:16,fontWeight:600,color:b.paid?"#5a5a7a":over?"#ef4444":"#fffffe",marginLeft:10,flexShrink:0,fontFamily:"'DM Sans',sans-serif"}}>${parseFloat(b.amount).toFixed(2)}</div>}
                      </div>
                      <div style={{fontSize:11,color:"#5a5a7a",marginBottom:4,fontFamily:"'DM Sans',sans-serif"}}>{b.category}</div>
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                        {b.dueDate&&<span style={{fontSize:11,color:over?"#ef4444":urgent?"#f59e0b":"#5a5a7a",fontFamily:"'DM Sans',sans-serif"}}>{over?"⚠ Overdue · ":urgent?"⚡ Due soon · ":""}{formatDate(b.dueDate)}</span>}
                        {b.repeat&&<span style={{fontSize:10,color:"#a78bfa",background:"rgba(167,139,250,0.12)",padding:"2px 7px",borderRadius:20,fontFamily:"'DM Sans',sans-serif"}}>🔄 {repeatDescription(b.dueDate,b.repeat,b.customDays)}</span>}
                        {adv&&<span style={{fontSize:11,color:"#f59e0b",background:"rgba(245,158,11,0.12)",padding:"2px 8px",borderRadius:20,fontFamily:"'DM Sans',sans-serif"}}>{adv}</span>}
                      </div>
                      {(b.advance||[]).length>0&&!b.paid&&<div style={{fontSize:10,color:"#3a3a5a",marginTop:3,fontFamily:"'DM Sans',sans-serif"}}>🔔 {b.advance.map(a=>ADVANCE_OPTIONS.find(x=>x.value===a)?.label).join(" · ")}</div>}
                      {b.note&&<div style={{fontSize:12,color:"#5a5a7a",marginTop:4,fontFamily:"'DM Sans',sans-serif"}}>{b.note}</div>}
                    </div>
                  ...
                  {!b.paid&&(
                    <div style={{marginTop:10,paddingTop:8,borderTop:"1px solid #1a1a2e",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:11,color:"#4a4a6a",fontFamily:"'DM Sans',sans-serif"}}>{b.repeat?`Next after paying: ${formatDate(nextDueDate(b.dueDate,b.repeat,b.customDays))}`:"One-time payment"}</span>
                      <button onClick={()=>markPaid(b.id)} className="btn" style={{background:"rgba(16,185,129,0.15)",border:"1px solid rgba(16,185,129,0.3)",borderRadius:8,padding:"4px 12px",color:"#10b981",fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>✓ Mark Paid</button>
                    </div>
                  )}
                </div>
              );
        ...
        {/* ── APPOINTMENTS ── */}
        {tab==="appointments"&&(showDetail?(
          <div className="fade">
            <button onClick={()=>setShowDetail(null)} style={{background:"transparent",border:"none",color:"#5a5a7a",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:13,marginBottom:16,padding:0,display:"flex",alignItems:"center",gap:6}}>← Back</button>
            <div style={{background:"#13131f",border:`1px solid ${tabColor.appointments}44`,borderRadius:16,padding:"20px",marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                <div style={{display:"flex",gap:12,alignItems:"center"}}>
                  <div style={{fontSize:32}}>{APPT_ICONS[showDetail.type]||"🏥"}</div>
                  <div>
                    <div style={{fontSize:18,fontWeight:500,color:"#fffffe"}}>{showDetail.doctorName}</div>
                    <div style={{fontSize:12,color:tabColor.appointments,fontFamily:"'DM Sans',sans-serif"}}>{showDetail.type}</div>
                  </div>
                </div>
                <button onClick={()=>{openEditA(showDetail);setShowDetail(null);}} style={{background:"rgba(56,189,248,0.1)",border:"1px solid rgba(56,189,248,0.3)",borderRadius:8,padding:"5px 12px",color:tabColor.appointments,fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Edit</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                {[["📅 Date",showDetail.date?`${formatDate(showDetail.date)}${showDetail.time?" at "+showDetail.time:""}`:null],["📍 Location",showDetail.location],["🩺 Reason",showDetail.reason],["📋 Follow-Up",showDetail.followUp?formatDate(showDetail.followUp):null]].filter(([,v])=>v).map(([label,val])=>(
                  <div key={label} style={{background:"#0f0e17",borderRadius:10,padding:"10px 12px"}}>
                    <div style={{fontSize:10,color:"#5a5a7a",letterSpacing:1,textTransform:"uppercase",marginBottom:4,fontFamily:"'DM Sans',sans-serif"}}>{label}</div>
                    <div style={{fontSize:13,color:"#fffffe",fontFamily:"'DM Sans',sans-serif"}}>{val}</div>
                  </div>
                ))}
              </div>
              {showDetail.diagnosis&&(
                <div style={{background:"rgba(56,189,248,0.07)",border:"1px solid rgba(56,189,248,0.2)",borderRadius:10,padding:"12px 14px",marginBottom:12}}>
                  <div style={{fontSize:10,color:tabColor.appointments,letterSpacing:2,textTransform:"uppercase",marginBottom:6,fontFamily:"'DM Sans',sans-serif"}}>🔬 Diagnosis</div>
                  <div style={{fontSize:14,color:"#fffffe",lineHeight:1.6,fontFamily:"'DM Sans',sans-serif"}}>{showDetail.diagnosis}</div>
                </div>
              )}
              {showDetail.notes&&(
                <div style={{background:"#0f0e17",borderRadius:10,padding:"12px 14px",marginBottom:12}}>
                  <div style={{fontSize:10,color:"#5a5a7a",letterSpacing:2,textTransform:"uppercase",marginBottom:6,fontFamily:"'DM Sans',sans-serif"}}>📝 Notes from Visit</div>
                  <div style={{fontSize:13,color:"#c0c0e0",lineHeight:1.8,whiteSpace:"pre-wrap",fontFamily:"'DM Sans',sans-serif"}}>{showDetail.notes}</div>
                </div>
              )}
              {showDetail.followUp&&(
                <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:10,padding:"10px 14px"}}>
                  <div style={{fontSize:12,color:"#f59e0b",fontFamily:"'DM Sans',sans-serif"}}>📅 Follow-up: <strong>{formatDate(showDetail.followUp)}</strong></div>
                </div>
              )}
            </div>
          </div>
          ...
          filtA.length===0?(
            <div className="pulse" style={{textAlign:"center",padding:"60px 0",color:"#4a4a6a"}}>
              <div style={{fontSize:40,marginBottom:8}}>🏥</div>
              <div style={{fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>{search?"No matches":"No appointments — tap + to add one"}</div>
            </div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {filtA.map(a=>{
                ...
                return(
                  <div key={a.id} className="card fade" style={{background:"#13131f",border:`1px solid ${soon?"rgba(56,189,248,0.3)":over?"#1a1a2e":"#2a2940"}`,borderLeft:`3px solid ${over?"#2a2940":soon?tabColor.appointments:"#2a2940"}`,borderRadius:12,padding:"12px 14px",opacity:over?0.6:1,cursor:"pointer"}} onClick={()=>setShowDetail(a)}>
                    <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                      <div style={{fontSize:24,flexShrink:0}}>{APPT_ICONS[a.type]||"🏥"}</div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:2}}>
                          <div style={{fontSize:15,color:"#fffffe",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",flex:1}}>{a.doctorName}</div>
                          {soon&&<span style={{fontSize:10,color:tabColor.appointments,background:"rgba(56,189,248,0.12)",padding:"2px 8px",borderRadius:20,flexShrink:0,marginLeft:8,fontFamily:"'DM Sans',sans-serif"}}>{days===0?"Today":days===1?"Tomorrow":`${days} days`}</span>}
                        </div>
                        <div style={{fontSize:11,color:tabColor.appointments,marginBottom:4,fontFamily:"'DM Sans',sans-serif"}}>{a.type}</div>
                        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                          {a.date&&<span style={{fontSize:11,color:over?"#5a5a7a":"#fffffe",fontFamily:"'DM Sans',sans-serif"}}>{formatDate(a.date)}{a.time&&" · "+a.time}</span>}
                          {a.location&&<span style={{fontSize:11,color:"#5a5a7a",fontFamily:"'DM Sans',sans-serif"}}>📍 {a.location}</span>}
                        </div>
                        {a.diagnosis&&<div style={{fontSize:11,color:"#38bdf8",marginTop:3,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontFamily:"'DM Sans',sans-serif"}}>Dx: {a.diagnosis}</div>}
                        {(a.notes||a.followUp)&&(
                          <div style={{display:"flex",gap:8,marginTop:4}}>
                            {a.notes&&<span style={{fontSize:10,color:"#5a5a7a",background:"#1a1a2e",padding:"2px 7px",borderRadius:10,fontFamily:"'DM Sans',sans-serif"}}>📝 Notes</span>}
                            {a.followUp&&<span style={{fontSize:10,color:"#f59e0b",background:"rgba(245,158,11,0.1)",padding:"2px 7px",borderRadius:10,fontFamily:"'DM Sans',sans-serif"}}>↩ {formatDate(a.followUp)}</span>}
                          </div>
                        )}
                      </div>
        ...
        {/* ── MEDICATIONS ── */}
        {tab==="medications"&&(filtM.length===0?(
          <div className="pulse" style={{textAlign:"center",padding:"60px 0",color:"#4a4a6a"}}>
            <div style={{fontSize:40,marginBottom:8}}>💊</div>
            <div style={{fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>{search?"No matches":filter==="Completed"?"No inactive medications":"No medications — tap + to add one"}</div>
          </div>
        ):(
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {filtM.map(m=>{
                ...
                <div key={m.id} className="card fade" style={{background:"#13131f",border:`1px solid ${!m.active?"#1a1a2e":refillOver||refillAdv?"rgba(167,139,250,0.35)":"#2a2940"}`,borderLeft:`3px solid ${!m.active?"#2a2940":"#a78bfa"}`,borderRadius:12,padding:"12px 14px",opacity:m.active?1:0.5}}>
                  <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                    ...
                    <div style={{flex:1,minWidth:0,cursor:"pointer"}} onClick={()=>openEditM(m)}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:2}}>
                        <div style={{fontSize:15,color:"#fffffe"}}>{m.name}</div>
                        <span style={{fontSize:11,color:m.active?"#a78bfa":"#5a5a7a",background:m.active?"rgba(167,139,250,0.12)":"#1a1a2e",padding:"2px 8px",borderRadius:20,flexShrink:0,marginLeft:8,fontFamily:"'DM Sans',sans-serif"}}>{m.active?"Active":"Inactive"}</span>
                      </div>
                      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:4}}>
                        {m.dosage&&<span style={{fontSize:12,color:"#fffffe",fontFamily:"'DM Sans',sans-serif"}}>{m.dosage}</span>}
                        {m.frequency&&<span style={{fontSize:12,color:"#5a5a7a",fontFamily:"'DM Sans',sans-serif"}}>· {m.frequency}</span>}
                      </div>
                      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                        {m.doctor&&<span style={{fontSize:11,color:"#5a5a7a",fontFamily:"'DM Sans',sans-serif"}}>👨‍⚕️ Dr. {m.doctor}</span>}
                        {m.startDate&&<span style={{fontSize:11,color:"#5a5a7a",fontFamily:"'DM Sans',sans-serif"}}>Started: {formatDate(m.startDate)}</span>}
                        {m.endDate&&<span style={{fontSize:11,color:"#5a5a7a",fontFamily:"'DM Sans',sans-serif"}}>Ends: {formatDate(m.endDate)}</span>}
                      </div>
                      {m.refillDate&&m.active&&<div style={{marginTop:4,fontFamily:"'DM Sans',sans-serif"}}><span style={{fontSize:11,color:refillOver?"#ef4444":refillAdv?"#f59e0b":"#5a5a7a"}}>{refillOver?"⚠ Refill overdue · ":refillAdv?"⏰ Refill soon · ":"💊 Refill: "}{formatDate(m.refillDate)}</span></div>}
                      {m.notes&&<div style={{fontSize:11,color:"#5a5a7a",marginTop:4,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontFamily:"'DM Sans',sans-serif"}}>{m.notes}</div>}
                    </div>
                    <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"center"}}>
                      <button onClick={()=>toggleMed(m.id)} className="btn" style={{background:m.active?"rgba(167,139,250,0.15)":"#1a1a2e",border:`1px solid ${m.active?"#a78bfa":"#2a2940"}`,borderRadius:8,padding:"4px 8px",color:m.active?"#a78bfa":"#5a5a7a",fontSize:10,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}}>{m.active?"Active":"Inactive"}</button>
      ...
      {showForm&&(
        <div className="modal-bg" onClick={()=>setShowForm(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",padding:16,zIndex:200,overflowY:"auto"}}>
          <div className="modal-box" onClick={e=>e.stopPropagation()} style={{background:"#13131f",border:`1px solid ${activeColor}44`,borderRadius:20,padding:"22px 20px",width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto",margin:"auto"}}>

            {/* REMINDER FORM */}
            {tab==="reminders"&&(<>
              <div style={{fontSize:18,marginBottom:18,color:activeColor}}>{editId?"Edit Reminder":"New Reminder"}</div>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  ...
                  <div><label style={LS}>Priority</label><select value={rForm.priority} onChange={e=>setRForm(f=>({...f,priority:e.target.value}))} style={IS}>{Object.entries(PRIORITIES).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</select></div>
                </div>
                {rForm.dueDate&&(<div>
                  <label style={LS}>🔔 Remind Me Before</label>
                  <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                    {ADVANCE_OPTIONS.map(a=>{const on=(rForm.advance||[]).includes(a.value);return(
                      <button key={a.value} onClick={()=>setRForm(f=>({...f,advance:on?f.advance.filter(x=>x!==a.value):[...f.advance,a.value]}))} className="pill" style={{padding:"5px 11px",borderRadius:20,fontSize:12,border:`1px solid ${on?"#f59e0b":"#2a2940"}`,background:on?"rgba(245,158,11,0.15)":"transparent",color:on?"#f59e0b":"#5a5a7a",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{a.label}</button>
                    );})}
                  </div>
                </div>)}
                <div style={{display:"flex",gap:10,marginTop:4}}>
                  <button onClick={()=>setShowForm(false)} style={{flex:1,background:"transparent",border:"1px solid #2a2940",borderRadius:10,padding:"11px",color:"#fffffe",fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
                  <button onClick={saveR} className="btn" style={{flex:1,background:rForm.title.trim()?activeColor:"#2a2940",border:"none",borderRadius:10,padding:"11px",color:"#fffffe",fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>{editId?"Save":"Add Reminder"}</button>
                </div>
              </div>
            </>)}
            ...
            {/* BILL FORM */}
            {tab==="bills"&&(<>
              <div style={{fontSize:18,marginBottom:18,color:activeColor}}>{editId?"Edit Bill":"New Bill"}</div>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                {bForm.dueDate&&bForm.repeat&&<div style={{background:"rgba(167,139,250,0.08)",border:"1px solid rgba(167,139,250,0.2)",borderRadius:10,padding:"9px 13px",fontSize:12,color:"#a78bfa",fontFamily:"'DM Sans',sans-serif"}}>🔄 {repeatDescription(bForm.dueDate,bForm.repeat,bForm.customDays)} · Next: <strong>{formatDate(nextDueDate(bForm.dueDate,bForm.repeat,bForm.customDays))}</strong></div>}
                ...
                <div style={{display:"flex",gap:10,marginTop:4}}>
                  <button onClick={()=>setShowForm(false)} style={{flex:1,background:"transparent",border:"1px solid #2a2940",borderRadius:10,padding:"11px",color:"#fffffe",fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
                  <button onClick={saveB} className="btn" style={{flex:1,background:bForm.title.trim()&&bForm.dueDate?activeColor:"#2a2940",border:"none",borderRadius:10,padding:"11px",color:"#fffffe",fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>{editId?"Save":"Add Bill"}</button>
                </div>
              </div>
            </>)}
            ...
            {/* APPOINTMENT FORM */}
            {tab==="appointments"&&(<>
              <div style={{fontSize:18,marginBottom:18,color:activeColor}}>{editId?"Edit Appointment":"New Appointment"}</div>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                <div><label style={LS}>Appointment Type</label>
                  <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                    {APPT_TYPES.map(t=>{const on=aForm.type===t;return(
                      <button key={t} onClick={()=>setAForm(f=>({...f,type:t}))} className="pill" style={{padding:"5px 10px",borderRadius:20,fontSize:12,border:`1px solid ${on?tabColor.appointments:"#2a2940"}`,background:on?"rgba(56,189,248,0.15)":"transparent",color:on?tabColor.appointments:"#5a5a7a",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{APPT_ICONS[t]} {t}</button>
                    );})}
                  </div>
                </div>
                ...
                <div style={{display:"flex",gap:10,marginTop:4}}>
                  <button onClick={()=>setShowForm(false)} style={{flex:1,background:"transparent",border:"1px solid #2a2940",borderRadius:10,padding:"11px",color:"#fffffe",fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
                  <button onClick={saveA} className="btn" style={{flex:1,background:aForm.doctorName.trim()&&aForm.date?activeColor:"#2a2940",border:"none",borderRadius:10,padding:"11px",color:"#fffffe",fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>{editId?"Save":"Add Appointment"}</button>
                </div>
              </div>
            </>)}
            ...
            {/* MEDICATION FORM */}
            {tab==="medications"&&(<>
              <div style={{fontSize:18,marginBottom:18,color:activeColor}}>{editId?"Edit Medication":"New Medication"}</div>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  <div><label style={LS}>Dosage</label><input value={mForm.dosage} onChange={e=>setMForm(f=>({...f,dosage:e.target.value}))} placeholder="e.g. 10mg" style={IS}/></div>
                  <div><label style={LS}>Frequency</label><select value={mForm.frequency} onChange={e=>setMForm(f=>({...f,frequency:e.target.value}))} style={IS}>{FREQ_OPTIONS.map(o=><option key={o} value={o}>{o}</option>)}</select></div>
                </div>
                ...
                {mForm.refillDate&&(<div>
                  <label style={LS}>🔔 Remind Before Refill</label>
                  <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                    {ADVANCE_OPTIONS.map(a=>{const on=(mForm.advance||[]).includes(a.value);return(
                      <button key={a.value} onClick={()=>setMForm(f=>({...f,advance:on?f.advance.filter(x=>x!==a.value):[...f.advance,a.value]}))} className="pill" style={{padding:"5px 11px",borderRadius:20,fontSize:12,border:`1px solid ${on?"#a78bfa":"#2a2940"}`,background:on?"rgba(167,139,250,0.15)":"transparent",color:on?"#a78bfa":"#5a5a7a",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{a.label}</button>
                    );})}
                  </div>
                </div>)}
                <div><label style={LS}>Notes / Side Effects</label><textarea value={mForm.notes} onChange={e=>setMForm(f=>({...f,notes:e.target.value}))} placeholder="e.g. Take with food, may cause drowsiness…" rows={3} style={{...IS,resize:"none",lineHeight:1.6}}/></div>
                <div style={{display:"flex",gap:10,marginTop:4}}>
                  <button onClick={()=>setShowForm(false)} style={{flex:1,background:"transparent",border:"1px solid #2a2940",borderRadius:10,padding:"11px",color:"#fffffe",fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancel</button>
                  <button onClick={saveM} className="btn" style={{flex:1,background:mForm.name.trim()?activeColor:"#2a2940",border:"none",borderRadius:10,padding:"11px",color:"#fffffe",fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>{editId?"Save":"Add Medication"}</button>
                </div>
              </div>
            </>)}
